...
const WeatherCard: React.FC<WeatherCardProps> = ({ weatherData }) => {
  ...
  return (
    <Card className='w-[400px] h-[400px] mx-auto shadow-md rounded-lg bg-white dark:bg-gray-900 text-gray-800 dark:text-gray-300 hover:scale-105 transition-transform duration-300'>
      ...
      <CardContent className='flex bg-gray-100 dark:bg-gray-900'>
        <div className='flex-1 flex flex-col items-center justify-center bg-white dark:bg-gray-800 p-4 rounded-lg shadow-md'>
          <div className='text-1xl font-bold mb-1 text-center'>
            <div>{dayOfWeek}</div>
            <div>{date}</div>
          </div>
          <img
